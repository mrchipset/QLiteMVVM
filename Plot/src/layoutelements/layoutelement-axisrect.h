...
public:
  ...
  Q_INVOKABLE int axisCount(QCPAxis::AxisType type) const;
  Q_INVOKABLE QCPAxis *axis(QCPAxis::AxisType type, int index=0) const;
  Q_INVOKABLE QList<QCPAxis*> axes(QCPAxis::AxisTypes types) const;
  Q_INVOKABLE QList<QCPAxis*> axes() const;
  Q_INVOKABLE QCPAxis *addAxis(QCPAxis::AxisType type, QCPAxis *axis=0);
  Q_INVOKABLE QList<QCPAxis*> addAxes(QCPAxis::AxisTypes types);
  Q_INVOKABLE bool removeAxis(QCPAxis *axis);
  Q_INVOKABLE QCPLayoutInset *insetLayout() const { return mInsetLayout; }
  
  void zoom(const QRectF &pixelRect);
  void zoom(const QRectF &pixelRect, const QList<QCPAxis*> &affectedAxes);
  Q_INVOKABLE void setupFullAxesBox(bool connectRanges=false);
  Q_INVOKABLE QList<QCPAbstractPlottable*> plottables() const;
  Q_INVOKABLE QList<QCPGraph*> graphs() const;
  Q_INVOKABLE QList<QCPAbstractItem*> items() const;
